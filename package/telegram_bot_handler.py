...
def handle_check_command(chat_id, bot_token, bucket_name, user_id):
    """Handle /check command - manually trigger campsite checking"""
    config = load_user_config(bucket_name, user_id)
    if not config:
        return send_telegram_message(chat_id, "❌ Error loading your configuration", bot_token)
    ...
    try:
        # Import camping functionality
        try:
            from camping import generate_human_output, check_park
        except ImportError:
            return send_telegram_message(
                chat_id,
                "❌ Camping module not available. Please contact support.",
                bot_token
            )
        ...
        for search_config in enabled_searches:
            try:
                ...
                # Check each park for this search
                info_by_park_id = {}
                ...
                if has_availabilities:
                    ...
                    # Send immediate notification for this search
        ...
        # Add any errors to summary
        errors = [r for r in results if r.get('error')]
        if errors:
            summary += f"\n\n⚠️ {len(errors)} search(es) had errors - check your search criteria."
        
        return send_telegram_message(chat_id, summary, bot_token)
